...
import { Users, Building2, ArrowRight, TrendingUp, CalendarDays } from "lucide-react"
...
const StatsCard = ({
  ...
  <div className="group relative col-span-1 flex overflow-hidden rounded-xl bg-white [box-shadow:0_0_0_1px_rgba(0,0,0,.03),0_2px_4px_rgba(0,0,0,.05),0_12px_24px_rgba(0,0,0,.05)] transform-gpu dark:bg-black dark:[border:1px_solid_rgba(255,255,255,.1)] dark:[box-shadow:0_-20px_80px_-20px_#ffffff1f_inset] h-40">
    ...
    <div className="flex items-center justify-center w-24 p-4">
      <Icon className={`h-12 w-12 ${color.replace('bg-', 'text-').replace('/50', '-600')} transition-all duration-300 ease-in-out group-hover:scale-75`} />
    </div>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Monthly email campaigns - Full year 2024</CardDescription>
      </CardHeader>
      <CardContent>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>New client acquisitions - Full year 2024</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="flex items-end justify-between gap-2 h-64">
  ...
  const getDateSubtext = () => {
    ...
    if (dateRange.preset) {
      switch (dateRange.preset) {
        case "today": return "Showing today's metrics"
        case "yesterday": return "Showing yesterday's metrics" 
        case "last7days": return "Showing last 7 days of data"
        case "last30days": return "Showing last 30 days of data"
        case "thisMonth": return "Showing current month's data"
        case "lastMonth": return "Showing previous month's data"
        default: return `Showing ${days} day${days > 1 ? 's' : ''} of data`
      }
    }
